...
5. ユーザー用リポジトリと店舗用リポジトリの両方で `git status -sb` を確認する。
6. 変更があるリポジトリは以下を順に実行する（変更がない場合は「変更なし」と報告してスキップ）。
7. 現在のブランチをコミットしてプッシュする。
8. その後、`main`にマージして`main`へpushする。
9. マージ後は元のブランチに戻す。
10. 元のブランチへ戻した時、そのブランチ名が今日の日付（`YYYY-mm-dd`）でない場合は、新しいブランチを`YYYY-mm-dd`形式で作成して切り替える。
11. `.gitignore`に含まれているもの以外は全てコミットする。
12. コミットメッセージは任意。
13. 依頼があった場合は両リポジトリを必ず確認し、変更がある場合は両方で同じ手順を実行する。
14. どちらか一方のみ変更がある場合でも、もう一方は「変更なし」を明示して報告する。
