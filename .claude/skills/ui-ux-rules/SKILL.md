...
- **【必須】トグルは必ず `CustomSwitchListTile`（`custom_switch_tile.dart`）を使うこと。**
...
- **ダイアログUIは `GameDialog`（`game_dialog.dart`）を使用する。** Flutterデフォルトの `showDialog` / `AlertDialog` の直接使用は禁止。
  ...
  - 店舗用: `lib/widgets/game_dialog.dart`（存在しない場合はユーザー用 `/Users/kanekohiroki/Desktop/groumapapp/lib/widgets/game_dialog.dart` を参考に同等実装を作成する）
