...
class NewsEditView extends ConsumerStatefulWidget {
  final NewsModel news;

  const NewsEditView({Key? key, required this.news}) : super(key: key);

  @override
  ConsumerState<NewsEditView> createState() => _NewsEditViewState();
}

class _NewsEditViewState extends ConsumerState<NewsEditView> {
  ...
  late DateTime _publishStartDate;
  late DateTime _publishEndDate;
  ...
  void initState() {
    ...
    _publishStartDate = widget.news.publishStartDate;
    _publishEndDate = widget.news.publishEndDate;
  }
  ...
  Future<void> _selectDate(BuildContext context, bool isStart) async {
    final initialDate = isStart ? _publishStartDate : _publishEndDate;
    final firstDate = isStart ? DateTime(2024) : _publishStartDate;
    ...
    if (picked != null) {
      setState(() {
        if (isStart) {
          ...
          if (_publishEndDate.isBefore(picked)) {
            _publishEndDate = picked.add(const Duration(days: 7));
          }
        } else {
    ...
    try {
      ...
      if (_selectedImage != null) {
        ...
        if (widget.news.imageUrl != null &&
            ...
            !widget.news.imageUrl!.startsWith('data:')) {
          try {
            await FirebaseStorage.instance.refFromURL(widget.news.imageUrl!).delete();
          } catch (e) {
            debugPrint('旧画像削除エラー: $e');
          }
        }
      ...
      } else if (_existingImageUrl == null && widget.news.imageUrl != null) {
        // 画像が削除された場合
        if (!widget.news.imageUrl!.startsWith('data:')) {
          try {
            await FirebaseStorage.instance.refFromURL(widget.news.imageUrl!).delete();
          } catch (e) {
    ...
    return DismissKeyboard(
      child: Scaffold(
        ...
        body: SafeArea(
          child: Column(
            children: [
              const CommonHeader(title: 'ニュースを編集'),
              Expanded(
