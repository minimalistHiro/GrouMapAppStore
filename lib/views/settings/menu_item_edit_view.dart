...
class _MenuItemEditViewState extends ConsumerState<MenuItemEditView> {
  ...
  Future<void> _loadOptionGroups() async {
    final optionGroupIds =
        (widget.menuItem['optionGroupIds'] as List<dynamic>?)
                ?.cast<String>() ??
            [];
    if (optionGroupIds.isEmpty) return;
    ...
    try {
      final snapshot = await FirebaseFirestore.instance
          ...
          .where(FieldPath.documentId, whereIn: optionGroupIds)
          .get();
      ...
      if (mounted) {
        ...
        final existingOverrides =
            (widget.menuItem['optionPriceOverrides']
                    as Map<String, dynamic>?) ??
                {};

        setState(() {
          _selectedOptionGroups = groups;
          _initOptionPriceOverrides(groups, existingOverrides);
